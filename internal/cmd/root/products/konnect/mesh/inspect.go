...
func newInspectDataplaneCmd(
	...
	cmdObj := &cobra.Command{
		...
		RunE: func(cobraCmd *cobra.Command, args []string) error {
			...
			// Read through configuration so a profile or environment variable
			// can set the usual inspection, with the flag winning.
			inspection := strings.TrimSpace(cfg.GetString(meshcommon.InspectTypeConfigPath))
			if inspection == "" {
				inspection = InspectPolicies
			}
			// Validated after resolution, so a configured value is checked the
			// same way a flag is rather than reaching the request unchecked.
	...
	cmdObj.Flags().String(TypeFlagName, InspectPolicies,
		fmt.Sprintf(`What to inspect. One of: %s.
- Config path: [ %s ]`, strings.Join(inspectDataplaneTypes, ", "), meshcommon.InspectTypeConfigPath))
...
func runInspectDataplane(helper cmd.Helper, name, inspection string) error {
	...
	return renderInspect(helper, collected, rows,
...
func runInspectPolicy(helper cmd.Helper, typeArg, name string) error {
	...
	// Paged like any other collection: a policy matching more dataplanes than
	// one page holds would otherwise be reported as matching fewer.
	...
	return renderInspect(helper, listPayload(envelope.Items), rows,
